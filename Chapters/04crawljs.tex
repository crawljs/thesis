...
\dirtree{%
  ...
  .4 \ldots.
  .3 url.js\DTcomment{URL Mapper (virtual-block) and utilities}.
...
Dispatcher.prototype.dispatch = function(urlObj) {
  ...
    ...
    , block = url.block(urlObj.hostname); //mapper
...
We referenced the ability of various configurable parameters of a worker. You find an example in ~\nameref{appendix:config.json}. The keys are pretty self explaining.
