...
\begin{algorithmic}[0]
\Function{Map}{$url$}\Comment{The url}
\State $host \gets url.host$\Comment{The host part of the url}
\State $hash \gets \textbf{hash}(url)$\Comment{Hash the host part}
\State $id \gets hash \bmod n$\Comment{limit answer between 0 and number of virtual blocks}
